...
**取数按优先级分档。** 贵的是 check 明细（每个 PR 上百条 check 记录），而只有**还开着的** PR
用得上它 —— 已合并 / 已关闭的不算 CI。所以还开着的每次刷新都拉全字段，已终结的只拉基本字段、
每次顺带带几个轮着来；想立刻看某个已终结的 PR，点它那行的刷新按钮单独拉。

**一次刷新发几条并发的小请求，不发一条大的。** 实测到 `api.github.com` 的 TLS 握手要 1–5 秒（TCP 只要 2ms），
而 `gh` 每调用一次就是新进程、新连接、新握手 —— 所以耗时几乎只跟请求个数有关，跟数据量无关
（同一个查询，1 个 PR 别名 5.1 秒，20 个 6.6 秒）。但**服务端**的开销要另算：每个 PR 要展开上百条 check 记录，一条请求装太多会被
GraphQL 网关判超时（实测 35 个 PR 挤一条，6 轮全 504）。而同时开太多连接又会被这条网络掐断
（7 条并发时 2 条直接 EOF）。所以取中间：**每条请求放 9 个 PR，全局同时最多 4 条请求**。
...
- **看板（创作者视角）** —— 我写的 PR 排到哪了：按功能归类、按先后关系组织成树，一眼看出该催哪个
- **评审视角** —— 别人卡在我这里的事：等我批准 workflow、请我 review、指派给我、@ 到我。
  按「卡得多死」排序，每行显示别人等了我多久。跨全部已配置仓库，不按项目分

刷新和设置中间的按钮切换两者，在贡献视角时按钮右上角显示待办数。
两个视角互不串门：看板已经在跟的 PR，评审清单里不会再出现一次。
一次刷新会把所有项目和两个视角全刷一遍。

后者只列**别人的** PR。我自己的归看板管；我已经批准过的也不列 —— 球在作者那边，我不是瓶颈了。

「请我 review」「等我批准」办完 GitHub 自己会撤掉。「@ 到我」是历史事实、不会自动消失，
所以规则是**有人在我最后一次发言之后又点了我才算还欠着** —— 提交 review、新加评论、回复他，
任何一种都算接手，不必回复那条 @ 本身。知会性质、根本不用回的，点行尾的「忽略」消掉，
下次再被 @ 还会出现。

「等我批准」还会额外确认 PR 有没有被别人合掉 —— workflow run 在 PR 合并后仍会挂在等待状态，
但那时流程已经走完，不该还堵在我这。

## 使用

**首次打开会自动走一遍聚光灯导览**：界面压暗，逐个把项目标签、描述块、状态、折叠区、
编辑开关、拖动手柄、刷新、设置圈亮出来，旁边一个气泡讲它是干什么的，一步一步点过去。
随时能「跳过」，之后想重看在设置的「上手导览」里点「重新查看」。

导览期间看板铺的是一份样板数据（编号 900+），不是你的 PR —— 这样第一次打开、
一条数据都没有的时候，每一步照样有真东西可指。
...
- **自动导入**：每次刷新会把该仓库下**作者是你、或指派给你**的 open PR 导进来，
  落在根级**最上面**；认得出父 PR 的挂到父块下面，位置**按编号倒序** ——
  新的排在旧的前面，跟根级「新的落最上面」是一个意思，免得新 backport
  沉到一堆已合并的下面。之后都可以靠拖拽调整。**每个 PR 只导入一次** —— 删掉之后不会被导回来，
...
- **拖拽**：编辑模式下拖动块左侧的手柄。落点决定意图：
  - 落在**块身上** = 成为它的子块，排在末尾（拖进描述块即归类，拖到 PR 下即表示「要等它先合并」）
  - 落在**块之间** = 插到那个位置，和缝下面那一行同级。**每一层都有缝** ——
    同级的先后顺序在哪一层都能调，从别处拖进缝里等于同时换爹 + 定位
  - 落在**底部虚线区** = 回到根级末尾
...
| 等依赖 | 靛 | 树上游还有没合并的 PR |
| CI 等批准 | 橙 | workflow 等维护者点「批准运行」—— 不点一步都不会跑，不是「在跑」 |
| CI 运行中 | 橙 | 等机器 |
| 等 review | 蓝 | 等人，可以去催 |
...
- **状态说明** —— 各状态的颜色、图标与含义对照表
- **更新** —— 检查更新、一键下载安装、启动时自动检查、是否自动安装
- **关于** —— 版本号、数据文件位置，以及重看上手导览

更新走 `gh` 拉 Release —— 匿名 API 请求限 60 次/小时且按 IP 计，很容易撞上限额；
`gh` 带认证，限额高得多。安装会校验下载包的签名，
然后替换 `/Applications` 里的 App 并重启。**「自动安装」默认关** —— 替换正在运行的
App 会让它重启，这种事不该在你不知情时发生。
