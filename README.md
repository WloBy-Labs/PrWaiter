...
**一次刷新只发一个请求。** 实测到 `api.github.com` 的 TLS 握手要 1–5 秒（TCP 只要 2ms），
而 `gh` 每调用一次就是新进程、新连接、新握手 —— 所以耗时几乎只跟请求个数有关，跟数据量无关
（同一个查询，1 个 PR 别名 5.1 秒，20 个 6.6 秒）。因此 PR 状态、CI 明细、自动导入用的两个
搜索全挤在一条 GraphQL 里，宁可多传 190 KB，也不多跑一次往返。
