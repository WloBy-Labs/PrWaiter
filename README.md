...
- **被顶替的 backport 会被清掉**：机器人常把 backport 关掉再重开一个，新旧两个 PR
  标题一字不差，留着就是白多一行。判定卡得很死 —— **父 PR 和目标分支都相同**、
  且这组里至少有一个没被关掉，才算被顶替。同一个改动打到 branch-3.5 和 branch-4.0
  是两件事，不会误伤；一组全关掉了也都留着（那说明这个 backport 是真的被放弃了）。
  已经在树上的也会在刷新时清一次，但**有子块或写过备注的一律不动**
