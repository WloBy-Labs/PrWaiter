...
- **添加 PR**：填编号点「添加 PR」，新 PR 一律先落在根级**最上面**，之后靠拖拽归类
  （新建的描述块同样放最上面）
...
| 状态 | 含义 |
| --- | --- |
| 已合并 / 已关闭 | 尘埃落定，淡化显示，可用「清理已合并」一键移除 |
| 草稿 | Draft PR，还没打算让人看 |
| CI 失败 | CI 红了 —— **该你动手**，所以排在「等依赖」前面 |
| 需修改 | reviewer 要求改 —— 同上，该你动手 |
| 等依赖 | 树上游还有没合并的 PR |
| CI 运行中 | 等机器 |
| 等 review | 等人 |
| 🟢 可合并 | 上游全合了 + 已批准 + CI 通过 —— **该去催了** |

「CI 失败」「需修改」排在「等依赖」前面是刻意的：你的 CI 红了就是红了，
上游合没合并不影响这件事该你去修。

描述块右侧按状态分项显示徽标（图标 + 数量），列表顶部有一条图例说明每个图标是什么状态。
